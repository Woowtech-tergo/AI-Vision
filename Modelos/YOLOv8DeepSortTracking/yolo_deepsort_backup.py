...
def process_webcam_frame(frame, detect_class, model_file):
    ...
    # Inicializa o modelo e o rastreador, se necessário
    if model is None:
        model = YOLO(model_file)
    if tracker is None:
        tracker = ds.DeepSort("deep_sort/deep_sort/deep/checkpoint/ckpt.t7")
    ...
    # Realiza o rastreamento dos objetos detectados com o modelo DeepSort.
    resultsTracker = tracker.update(detections, confarray, frame_bgr)

    for x1, y1, x2, y2, Id in resultsTracker:
        x1, y1, x2, y2 = map(int, [x1, y1, x2, y2])  # Converte as posições para inteiros.

        # Desenha bounding boxes e texto
        cv2.rectangle(frame_bgr, (x1, y1), (x2, y2), (255, 0, 255), 3)
        putTextWithBackground(
            frame_bgr,
            str(int(Id)),
            (max(-10, x1), max(40, y1)),
    ...
    # Converte a imagem de volta para RGB antes de retornar
